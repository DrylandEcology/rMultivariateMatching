##########################################
#
# Purpose: Select matching variables and determine matching criteria
#
...
####################################################################
# Step 3: Run algorithm with different matching criteria and check coverage
# Choose a value for k-points near the mid-point of the range of k to be tested later

# Note: if areal coverage is near 100%, try smaller matching criteria for more precise matches
# or, if areal coverage is very low, try larger matching critera to try to cover more area

# After each solution for k is complete, algorithm will generate a plot of 
# proportion of area covered ~ number of iterations
# This will verify that the areal coverage stopping criterion stops the algorithm (instead of # of iterations)
...
n_starts = 10 # Number of random starts (k randomly selected points)
# Create a vector of k's to try:
klist <- 200
...
best_area/totalarea
# 0.8925198 for our run
# best_area will differ due to variation in random selection of k cells for each start

# After some experimentation, we decided to use 10% of the range of each variable for matching,
# except 5% of range for bioclim_01 (mean annual temperature) and bioclim_12 (mean annual precipitation)
...
# "clay" = depth-weighted percentage clay (3.8)
# "sand" = depth-weighted percentage sand (5.3)