...
* `raster_template` This is one of the rasters used to define the **target cells** and it is used as a template to determine the area represented by each set of **matching criteria**.
...
#### Example of soil types used in simulation
...
* `subset_in_target` A boolean indicating if the **subset cells** are taken from within the **target cells**. Currently there is only functionality for `FALSE`. 
...
* `matches` A data frame output from the `multivarmatch` or `secondaryMatching` function.
...
* `subset_in_target` A boolean indicating if the **subset cells** are taken from within the **target cells**. (`FALSE` in this example). 
...
* `secondarymatch` A boolean indicating if a secondary matching step should be completed. `TRUE` in this example.
...
* `matches` A data frame output from the `multivarmatch` or `secondaryMatching` functions.
...
* `quality_name` The name of the column in the `matches` data frame that contains the matching quality for the **subset cells** matched to each **target cell**. Must be either "matching_quality" or "matching_quality_secondary". (Designated as "matching_quality" in this example).
